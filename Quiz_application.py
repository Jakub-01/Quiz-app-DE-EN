...
quest = get_file("DE-EN.xlsx",2)
answ = get_file("DE-EN.xlsx",1)
user_highscore = 0
user_name = None
...
def begin_test():
    ...
    if user_name == None:
        user_name = simpledialog.askstring("Enter Your Name", "Please enter your name:")
        if user_name == "":
            user_name = "Guest"
        else:
            pass
    ...
    begin_next_button = tkinter.Button(window, text = 'Next question!', padx =30, pady=12, command = begin_test)
    begin_next_button.grid(row = 1, column = 0,columnspan=2)
    begin_next_button.config(state = 'disabled')
    check_answer_button.grid_remove()
    check_answer_button = tkinter.Button(window, text = "Check!",padx = 30, pady=12, command = check_answer)
...
def check_answer():
    ...
    if len(user_answer) > 0:
        ...
        check3 = False
    #checking if all three conditions are met. If so, user gets a point, if not, he doesn't, but he can still move on to the next question, by clicking "Next question!" button.       
    if all([check1,check2,check3]):
        score += 1
        check_answer_button = tkinter.Button(window, text = "Correct answer!"
        ,padx =30, pady=12, bg = '#32CD32', fg = 'black' , command = check_answer)
        check_answer_button.grid(row = 5, column = 0, columnspan=2)
    else:
        check_answer_button = tkinter.Button(window, text = "Wrong answer :("
...
window.title("Quiz - 1000 German words")

#defining GUI elements
welcome_message = tkinter.Label(window, text = "Quiz - 1000 German words")
translateRequest = tkinter.Label(window, text = "Translate:")
question_label = tkinter.Label(window, text = "")
answerPrompt = tkinter.Label(window, text = "Your answer:")
answer = tkinter.Entry(window, width=25, borderwidth = 5)
begin_next_button = tkinter.Button(window, text = "Let's begin", padx =30, pady=12, command = begin_test)
correctAnswerPrompt = tkinter.Label(window, text = "Correct answer:")
check_answer_button = tkinter.Button(window, text = "Check!",padx =30, pady=12, command = check_answer,state='disabled')
answer_label = tkinter.Label(window, text = " ",wraplength=160) 
N_label = tkinter.Label(window, text = "Number of attempts:")
score_indicator_label = tkinter.Label(window, text = "Your score:")
percentagePrompt = tkinter.Label(window, text = "Correct answer %:")
N_indicator = tkinter.Label(window, text = N)
score_indicator = tkinter.Label(window, text = score)
percentage = tkinter.Label(window, text = correct_answer_ratio)
user_label = tkinter.Label(window, text = "User:")
user_highscore_label = tkinter.Label(window, text = "Highscore:")
user_indicator = tkinter.Label(window, text = user_name)
user_highscore_indicator = tkinter.Label(window, text = user_highscore)
